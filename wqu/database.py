...
class MongoRepository:
    ...
    def __init__(
        self,
        client=MongoClient(host="192.126.39.2", port=27017),
        db="wqu-abtest",
        ...
        client : pymongo.MongoClient, optional
            By default MongoClient(host=“ip_of_mongo_device”, port=27017)
        db : str, optional
    ...
    def get_nationality_value_counts(self, normalize=True):
        ...
        df_nationality = (pd.DataFrame(result).rename({"_id":"country_iso2"}, axis = 1).sort_values("count"))
    ...
    def get_ages(self):
        ...
        result = self.collection.aggregate([{
            "$project": {
                "years": {"$dateDiff": {"startDate":"$birthday",
                                        "endDate":"$$NOW",
                                        "unit":"year"}
                         }
                }
    ...
    def get_ed_value_counts(self, normalize=False):
        ...
        education = (
            pd.DataFrame(result)
            .rename({"_id":"highestDegreeEarned"}, axis=1)
            .set_index("highestDegreeEarned")
            .squeeze()
        )
